...
public class ShardedSessionImpl implements ShardedSession, ShardedSessionImplementor,
  ...
  public FlushMode getFlushMode() {
    // all org.hibernate.shards must have the same flush mode
    Session someSession = getSomeSession();
    if (someSession == null) {
      someSession = shards.get(0).establishSession();
    }
  ...
  public CacheMode getCacheMode() {
    // all org.hibernate.shards must have the same cache mode
    Session someSession = getSomeSession();
    if (someSession == null) {
      someSession = shards.get(0).establishSession();
    }
  ...
  public void cancelQuery() throws HibernateException {
    // cancel across all org.hibernate.shards
    for (Shard shard : shards) {
      if (shard.getSession() != null) {
  ...
  ShardId getShardIdOfRelatedObject(Object obj) {
    ...
    for(Pair<Type, Object> pair : CrossShardRelationshipDetectingInterceptor.buildListOfAssociations(types, values)) {
      if(pair.getFirst().isCollectionType()) {
        ...
        /**
         * if we're not checking for different org.hibernate.shards, return as soon as we've
         * got one
         */
  ...
  public Query createFilter(Object collection, String queryString)
    ...
    Session session;
    if(shard == null) {
      // collection not associated with any of our org.hibernate.shards, so just delegate to
