...
import com.vio.messaging.IMessageRespond;
import com.vio.messaging.MessagingHandler;
...
public class TasksExecutor implements MessageListener {
    ...
    protected TaskResult executePersistentTask( Class<? extends IPersistentTask> taskClass, Message message ) throws ExecutionException {
        ...
        try {
            ...
            if ( new Date().getTime() - taskInstance.getLastExecutionTime() < Registry.getServerConfig().getTaskAfterFailDelay() ) {
    ...
    private boolean validateTask( IPersistentTask task ) throws ExecutionException, ConfigReaderException {
        if ( task.getExecutionFails() > Registry.getServerConfig().getTaskFailsLimit() ) {
            try {
                task.remove();
                return false;
            } catch ( Throwable e ) {
